...
See \exampleref{2dgrf} below.

\begin{example}[float=hp]{2-Dimensional Gaussian Random Field}{2dgrf}
...
including the squared exponential, or Gaussian, kernel,
...
\begin{definition}{Symmetric Wigner Random Matrix}{wigner}
	...
	if its independent entries $M_{ij}$ satisfy the following:
	\begin{align*}
		\forall i,j &\suchthat 1 \leq i < j < n\\
			&\Ex{M_{i, j}} = 0\\
			&\Ex{M_{i, j}^2} = 1/\sqrt{n}\\
			&M_{j, i} = M_{i, j} \text{\ and\ } M_{i, j} \text{\ i.i.d}\\
		\forall i\\
			&\Ex{M_{i, i}} = \gamma\\
			&\Ex{M_{i, i}^2} = 1/\sqrt{n}\\
			&M_{i, i} \text{\ i.i.d}
	\end{align*}
...
\begin{figure}[ht]
	...
	\end{center}
	\caption{\textbf{The Wigner Semicircular Distribution Does Not Match
	the Spectrum of Neural Network Hessians}}{\textbf{A}:
	\textbf{B}:}
...
As $\gamma$ changes, this control parameter shifts.
...
\begin{definition}{Cokernel}{cokernel}
	...
	\end{equation}
	This subspace is the orthogonal complement of $\ker M$
	(see \defref{kernel}).
\end{definition}
...
In \exampleref{gp}, it was noted that
