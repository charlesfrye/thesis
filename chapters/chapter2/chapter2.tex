...
In \sectionref{exact},
solved the Newton system exactly
\begin{equation}
	0 = \grad{f}{\theta} + \hess{f}{\theta}p^\star
\end{equation}
...
The classic choice for linear solver is the conjugate gradient algorithm.
This algorithm has several nice properties.
With exact arithmetic, it produces an exact solution
in exactly $N$ steps, where the dimension of the system is $N\times N$.
Furthermore, it can be implemented entirely
...
These properties are shared by the class of
Krylov-subspace methods,
which compute their solutions in a sequence of subspaces
